...
public class UserPlanController {
    ...
    public Result<?> addUserPlan(String planContext, Timestamp startTime,  @RequestHeader("H-User-Id") String userId) {
        if (planContext == null){
        ...
        userPlanMapper.insert(new UserPlan(planContext, startTime, Long.valueOf(userId)));
        return Result.success();
    }
    @PostMapping("/deleteUserPlan")
    public Result<?> deleteUserPlan(Long planId, @RequestHeader("H-User-Id") String userId) {
    ...
    public Result<?> startUserPlan(Long planId, @RequestHeader("H-User-Id") String userId) {
        QueryWrapper<UserPlan> queryWrapper = new QueryWrapper<>();
        queryWrapper.eq("id", planId);
        queryWrapper.eq("user_id", userId);
        if (userPlanMapper.selectOne(queryWrapper) == null){
    ...
    public Result<?> endUserPlan(Long planId, @RequestHeader("H-User-Id") String userId) {
        QueryWrapper<UserPlan> queryWrapper = new QueryWrapper<>();
        queryWrapper.eq("id", planId);
        queryWrapper.eq("user_id", userId);
        if (userPlanMapper.selectOne(queryWrapper) == null){
    ...
    public Result<?> getUserPlans(@RequestHeader("H-User-Id") String userId) {
        QueryWrapper<UserPlan> queryWrapper = new QueryWrapper<>();
        queryWrapper.eq("user_id", userId);
